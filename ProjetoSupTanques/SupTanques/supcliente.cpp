...
SupCliente::SupCliente()
    : meuUsuario(""), encerrarCliente(true), is_admin(false), last_S(), start_t(time_t(-1)), last_t(time_t(-1)), timeRefresh(20)
      /* ACRESCENTAR */
      ,
      tcp_mysocket_cli(), mtx(), thr_cliente()
{
...
SupCliente::~SupCliente()
{
  ...
  if (isConnected())
  {
    ...
    /* ACRESCENTAR */
    tcp_mysocket_cli.write_uint16(CMD_LOGOUT);
                          ...
                          const std::string &Senha)
{
  ...
  try
  {
    ...
    /* ACRESCENTAR */
    if (tcp_mysocket_cli.connect(IP, SUP_PORT) != mysocket_status::SOCK_OK)
      throw 102;
    ...
    /* ACRESCENTAR */
    if (tcp_mysocket_cli.write_uint16(CMD_LOGIN) != mysocket_status::SOCK_OK)
      throw 103;

    // Envia 1o parametro do comando (login)
    // Em caso de erro, throw 104
    /* ACRESCENTAR */
    if (tcp_mysocket_cli.write_string(Login) != mysocket_status::SOCK_OK)
      throw 104;

    // Envia 2o parametro do comando (senha)
    // Em caso de erro, throw 105
    /* ACRESCENTAR */
    if (tcp_mysocket_cli.write_string(Senha) != mysocket_status::SOCK_OK)
      throw 105;

    // Leh a resposta (cmd) do servidor ao pedido de conexao
    // Em caso de erro, throw 106
    /* ACRESCENTAR */
    if (tcp_mysocket_cli.read_uint16(cmd, SUP_TIMEOUT * 1000) != mysocket_status::SOCK_OK)
      throw 106;
    ...
    /* ACRESCENTAR */
    thr_cliente = std::thread([this]()
                              { this->main_thread(); });
    if (!thr_cliente.joinable())
      throw 108;

    // A interface serah exibida no modo conectado na primeira iteracao da thread
  }
  catch (int err)
  {
    // Encerra o cliente
    ...
    // A interface permanece exibida no modo desconectado
  }
}
...
void SupCliente::setValvOpen(bool isV1, bool Open)
{
  ...
  try
  {
    ...
    /* ACRESCENTAR */
    if (tcp_mysocket_cli.write_uint16(cmd) != mysocket_status::SOCK_OK)
      throw 202;

    // Escreve o parametro do comando (==0 se fechada !=0 se aberta)
    // Em caso de erro, throw 203
    /* ACRESCENTAR */
    uint16_t estado = (Open ? 1 : 0);
    if (tcp_mysocket_cli.write_uint16(estado) != mysocket_status::SOCK_OK)
      throw 203;
...
void SupCliente::setPumpInput(uint16_t Input)
{
  ...
  try
  {
    ...
    /* ACRESCENTAR */
    if (tcp_mysocket_cli.write_uint16(CMD_SET_PUMP) != mysocket_status::SOCK_OK)
      throw 302;

    // Escreve o paramentro do comando CMD_SET_PUMP (Input = 0 a 65535)
    // Em caso de erro, throw 303
    /* ACRESCENTAR */
    if (tcp_mysocket_cli.write_uint16(Input) != mysocket_status::SOCK_OK)
      throw 303;
...
void SupCliente::main_thread(void)
{
  ...
  while (!encerrarCliente && isConnected())
  {
    ...
    try
    {
      ...
      /* ACRESCENTAR */
      if (tcp_mysocket_cli.write_uint16(CMD_GET_DATA) != mysocket_status::SOCK_OK)
        throw 401;
      ...
      // Se resposta nao for CMD_DATA, throw 403
      if (cmd != CMD_DATA)
        throw 403;
      // Leh os dados (com timeout)
      // Em caso de erro, throw 404
      /* ACRESCENTAR */
      if (tcp_mysocket_cli.read_bytes((mybyte *)&S, sizeof(SupState), SUP_TIMEOUT * 1000) != mysocket_status::SOCK_OK)
        throw 404;
