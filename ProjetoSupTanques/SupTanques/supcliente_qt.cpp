...
void SupClienteQt::slotExibirInterface()
{
  ...
  if (estahConectado)
  {
    ...
    if (estavaConectado <=0)
    {
      ...
      QString msg = QString(" CONNECTED: ") + /* MODIFICAR */ QString::fromStdString(meuUsuario) +
          " (" + (ehAdministrador ? "admin" : "viewer") + ")";
      statusMsg->setText(msg);
    }

    // Exibe nos visualizadores o ultimo estado lido da planta.
    showValves(/* MODIFICAR */ lastState().V1, /* MODIFICAR */ lastState().V2);
    showPump(/* MODIFICAR */ lastState().PumpInput);
    showH(/* MODIFICAR */ lastState().H1, /* MODIFICAR */ lastState().H2, /* MODIFICAR */ lastState().ovfl);
    showFlow(/* MODIFICAR */ lastState().PumpFlow);
